...
export const randomAddress = () => addressSchema.parse(ethers.Wallet.createRandom().address)

export const createTagAddress = <T extends string, A extends Address>(tag: T, address: A): TagAddress<T> =>
  `${tag}:${address}` as unknown as TagAddress<T>

export const randomTagAddress = <T extends string>(tag: T): TagAddress<T> => createTagAddress(tag, randomAddress())
